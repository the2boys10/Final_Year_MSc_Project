...
/**
 * A search technique that penalised the cost of turning as well as moving through other predators if turning takes a full turn, looks for possible prey locations
 */
public class BFSsearchFound
{
	/**
	 *
										 ...
										 LinkedList<Cell> currentBound, Robot superc)
	{
		if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc) || Util.checkContentsWithoutPred(xCordCurrent, yCordCurrent, superc))
		{
			...
			{
				superc.map[xCordCurrent][yCordCurrent].distance = 20;
			}
			else
			{
	...
	{
		// if the cell we would like to expand is a predator and the distance = distance + 20
		if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc) &&
				superc.map[xCordCurrent][yCordCurrent].contents != superc.ID &&
		...
		// if the cell we would like to expand is empty and the distance = distance + 1
		else if (Util.checkContentsWithoutPred(xCordCurrent, yCordCurrent, superc) &&
