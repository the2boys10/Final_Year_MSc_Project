 ...
 */
public class BFSsearchTurnCostFoundNoIncentive
{
    /**
     ...
     * @param offTurn      The value to add to the distance depending on if the robot would have to turn
    ...
    private static void initialiseSearch(int xCordCurrent, int yCordCurrent, int fromWhere,
                                         LinkedList<Cell> currentBound, int offTurn,
                                         Robot superc)
    {
		int indexOf = 10;
		if(fromWhere==1)
        ...
        // if the cell we wish to initialise the search is a predator or empty then add it to the list of cells we wish to expand.
    }
    ...
    public static int search(int xCord, int yCord, char orientation, Robot superc)
    {
        ...
        while (currentBound.size() > 0)
        {
            ...
            // if the cell could possibly have prey then update the current distance and movements
            ...
            else
            {
                ...
                expand(current, current.xCord, current.yCord, current.distance, current.fromWhere, currentBound, superc, current.turnFrom);
                Collections.sort(currentBound, new Comparator<Cell>()
                {
                    public int compare(Cell x, Cell y)
                    {
     ...
     * @param offTurn        The value to add to the distance depending on if the robot would have to turn
     * @param fromWhereEnter The moves which reached the cell in binary form for the previous step <ul><li>1:N</li><li>2:E</li><li>4:S</li><li>8:W</li></ul>
     */
