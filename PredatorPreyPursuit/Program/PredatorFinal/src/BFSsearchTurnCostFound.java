 ...
 * through other predators if turning takes a full turn, looks for possible prey
 * locations
 */
public class BFSsearchTurnCostFound {
	/**
	 *
	 * @param xCordCurrent the xCord of the cell we would like to add to the search
	 *                     list
	 * @param yCordCurrent the yCord of the cell we would like to add to the search
	 *                     list
	 * @param fromWhere    The moves which reached the cell in binary form for the
	 *                     first step
	 *                     <ul>
	 *                     <li>1:N</li>
	 *                     <li>2:E</li>
	 ...
	 * @param offTurn      The value to add to the distance depending on if the
	 *                     robot would have to turn
	...
	private static void initialiseSearch(int xCordCurrent, int yCordCurrent, int fromWhere,
		...
		if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc)
			...
			if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc)) {
				superc.map[xCordCurrent][yCordCurrent].fromWhere2[indexOf] = 20 + offTurn;
				superc.map[xCordCurrent][yCordCurrent].distance = 20 + offTurn;
	 ...
	 * @param orientation The orientation that the robot is currently facing
	 * @param superc      The Robot that is calling.
	...
	public static int search(int xCord, int yCord, char orientation, Robot superc) {
		...
		while (currentBound.size() > 0) {
			...
			// if the cell could possibly have prey then update the current distance and
			// movements
	 ...
	 * @param offTurn        The value to add to the distance depending on if the
	 *                       robot would have to turn
	 * @param fromWhereEnter The moves which reached the cell in binary form for the
	 *                       previous step
	 *                       <ul>
	 *                       <li>1:N</li>
	 *                       <li>2:E</li>
	...
	private static void expandOne(int xCordCurrent, int yCordCurrent, int distance, int fromWhere,
		...
		if (fromWhereEnter == 1)
			...
			indexOf = 3;
		if (Util.checkContentsWithoutPred(xCordCurrent, yCordCurrent, superc)
		...
		} else if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc)
				&& (superc.map[xCordCurrent][yCordCurrent].distance >= distance + 19)) {
			superc.map[xCordCurrent][yCordCurrent].fromWhere3[indexOf] |= fromWhere;
			superc.map[xCordCurrent][yCordCurrent].fromWhere2[indexOf] = distance + 20;
			if (superc.map[xCordCurrent][yCordCurrent].distance == 1000) {
				currentBound.add(superc.map[xCordCurrent][yCordCurrent]);
			}
			if (superc.map[xCordCurrent][yCordCurrent].distance > distance + 20) {
				superc.map[xCordCurrent][yCordCurrent].distance = distance + 20;
			}
		}
	}
	...
	private static int[] findLowestCost(Cell current, int direction) {
		...
		if (choseToTurn)
			return new int[] { initialTurn, lowestCost + 1 };
		else
			return new int[] { initialTurn, lowestCost };
	}
}
