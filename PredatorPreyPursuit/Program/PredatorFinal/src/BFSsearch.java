...
/**
 * A search technique that penalised the cost of turning as well as moving through other predators if turning takes a full turn, looks for un-searched tiles
 */
public class BFSsearch
{
	...
	{
		// if the cell we wish to initialise the search is a predator or empty
		if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc) || Util.checkContentsWithoutPred(xCordCurrent, yCordCurrent, superc))
		{
			...
			// if the cell we wish to initialise the search is a predator
			if (Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc))
			{
				superc.map[xCordCurrent][yCordCurrent].distance = 4;
			}
			else
			{
	...
	private static void expandOne(int xCordCurrent, int yCordCurrent, int distance, int fromWhere, LinkedList<Cell> currentBound, Robot superc)
	{
		...
		if ((Util.checkContentsWithPred(xCordCurrent, yCordCurrent, superc) || Util.checkContentsWithoutPred(xCordCurrent, yCordCurrent, superc)) && (superc.map[xCordCurrent][yCordCurrent].distance == distance + 1 ||
