...
/**
 * A search technique that penalised the cost of turning if turning takes a full
 * turn, looks for un-searched tiles
 */
public class BFSsearchTurnCostNoIncentive {
	/**
	 *
	...
	public static int search(int xCord, int yCord, char orientation, Robot superc) {
		...
		while (currentBound.size() > 0) {
			...
			// if the cell is un-searched then update the current distance and movements
			if (!current.flagged) {
				int lowestCost = 1000;
				int initialTurn = 0;
	...
	private static void expandOne(int xCordCurrent, int yCordCurrent, int distance, int fromWhere,
		...
		// if the cell we would like to expand is a predator or empty and we need to
		// turn to make it into the cell and the distance = distance + 2
		int indexOf = 10;
		if (fromWhereEnter == 1)
			indexOf = 0;
